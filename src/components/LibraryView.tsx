...
export const LibraryView = ({ 
  ...
  return (
    <div className="space-y-6 pb-24">
      ...
      {/* Hero Story Card (if exists) */}
      {heroStory && (
        <div>
          <h3 className="text-sm font-bold text-slate-400 uppercase tracking-wider mb-3">Active Journey</h3>
          <div 
          ...
          >
            <img 
              src={heroStory.pages[0]?.images?.[0]?.url || DEFAULT_IMAGES[0]} 
              style={{ filter: FILTERS[heroStory.settings?.filter || 'none'].style }} 
      ...
      {/* Horizontal Carousels for Stories */}
      {hasStories && allStories.length > 0 && (
        <div className="space-y-6">
          <div>
            <h3 className="text-sm font-bold text-slate-400 uppercase tracking-wider mb-3">New in Your Circles</h3>
            <div className="flex gap-3 overflow-x-auto pb-2 scrollbar-hide">
              {allStories.slice(0, 6).map(s => (
                <StoryCard key={s.id} story={s} onRead={onReadStory} onEdit={onEditStory} compact />
              ))}
            </div>
          </div>
          ...
          {activeCategory === 'Featured' && (
            <div>
              ...
              <div className="flex gap-3 overflow-x-auto pb-2 scrollbar-hide">
                {allStories.slice(0, 6).map(s => (
                  <StoryCard key={`bedtime-${s.id}`} story={s} onRead={onReadStory} onEdit={onEditStory} compact />
                ))}
              </div>
            </div>
          )}
      ...
      {hasStories && (
        <div>
          <h3 className="text-sm font-bold text-slate-400 uppercase tracking-wider mb-3">All Stories</h3>
          <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3">
            {allStories.map(s => (
              <StoryCard key={s.id} story={s} onRead={onReadStory} onEdit={onEditStory} />
            ))}
          </div>
        </div>
      )}
...
// Helper Component: Reusable Story Card
...
const StoryCard = ({ story, onRead, onEdit, compact }: StoryCardProps) => (
  <div 
  ...
  >
    ...
    >
      <img 
        ...
        style={{ filter: FILTERS[story.settings?.filter || 'none'].style }} 
        className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-105" 
        alt={story.title}
      />
    ...
    <div className="p-3 space-y-2">
      ...
      <p className="text-slate-400 text-xs line-clamp-1">
        {story.tagline || story.pages[0]?.text?.substring(0, 40) || 'A magical adventure'}...
      </p>
